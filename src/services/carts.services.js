...
const updateQuantityProductInCart = async (cid, pid, quantity) => {
  const cart = await cartsRepository.getCartById(cid); // Asegúrate de obtener el carrito primero
  if (!cart) throw customErrors.cartNotFoundError();

  const product = await productsRepository.getById(pid); // Obtén el producto
